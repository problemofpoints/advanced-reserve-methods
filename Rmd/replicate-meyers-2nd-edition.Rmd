...
```
```{r sim-predictions}
sigma_10 <- spread_draws(csr_fit, sig[origin_lag]) %>% 
  ungroup() %>% 
  filter(origin_lag == max(origin_lag)) %>% 
  select(.draw, sig)

illust_sim_params <- spread_draws(csr_fit, logelr, alpha[origin_lag]) %>% 
  ungroup() %>% 
  left_join(sigma_10, by = ".draw") %>% 
  left_join(illust_data_stan %>% distinct(origin_lag, logprem), by = "origin_lag")

illust_sim_results <- illust_sim_params %>% 
  mutate(.value = rlnorm(nrow(illust_sim_params), logprem + logelr + alpha, sig))
```
  ...
  bind_rows(
    illust_sim_results %>% 
      ...
      summarise(estimate = mean(.value),
                se = sd(.value), 
                cv = se / estimate) %>% 
      add_column(origin_lag = NA, .before = 1)
  )
...
- prior predicive checks? test impact of prior distributions
- add alternative speedup formulations

```{r run-csr-illustrative2}
