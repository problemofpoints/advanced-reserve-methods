...
- use `brms` to get optimized code
- add prior parameters as data in stan
...
csr_fit2 <- sampling(stan_model_csr2,
                    iter = 5000,
                    data = c(stan_data, list(prior_only = FALSE)),
...
csr_fit2_prior <- sampling(stan_model_csr2,
                    iter = 2000,
                    data = c(stan_data, list(prior_only = TRUE)),
                    pars = c("r_alpha", "r_beta"), include = FALSE,
                    init = init_vals, 
                    seed = 12345, 
                    control = list(adapt_delta = 0.999, max_treedepth = 50))

illust_csr_param_sims_prior <- gather_draws(csr_fit2_prior, logelr, alpha[origin_lag], beta[origin_lag], gamma, sig[origin_lag])
...
```
```{r ppc-sims}
illust_sim_params_prior <- spread_draws(csr_fit2_prior, logelr, alpha[origin_lag], 
                                      beta[dev_lag], speedup[dev_lag], sig[dev_lag]) %>% 
  ungroup() %>% 
  left_join(illust_data_stan %>% distinct(origin_lag, logprem), by = "origin_lag")
...
```{r brms}
csr_brms <- brm(logloss ~ 1 + offset(logprem) + factor(origin_lag) + factor(dev_lag),
                data = illust_data_stan,
                family = gaussian(), 
                save_model = here::here("stan/csr_brms.stan"))
