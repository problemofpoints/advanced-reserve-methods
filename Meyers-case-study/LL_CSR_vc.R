#
# R/Stan Script that fits the CSR model to data underling the
...
AY.Exhibit=NULL
#AY.Exhibit=paste(run_id,"AY_Exhibit_CSR-cv.csv")
parm.summary=NULL
#parm.summary=paste(run_id,"Parm_Summary_CSR-cv.csv")
#
# read in data
#
...
init.CSR=function(chain_id){
  ...
  list(r_alpha=rnorm(W-1,0,0.2),r_beta=runif(D-1),a_ig=runif(D),
       logelr=runif(1,-0.75,-0.5),r_gamma=rnorm(length(keep_cal)-1,0,0.025))
}
#
# Stan CSR script
#
...
real r_gamma[C-c];
real logelr;
real <lower=0,upper=100000> a_ig[D];
}
...
for (i in 1:(C-1))
 if (i<c) gamma[i]=0;
 else gamma[i]=r_gamma[i-c+1];
speedup[C]=1;
for (i in 1:(C-1)) speedup[C-i]=speedup[C-i+1]*(1+gamma[C-i]);
sig2[D] = gamma_cdf(1/a_ig[D],1,1);
for (i in 1:(D-1)) sig2[D-i] = sig2[D+1-i]+gamma_cdf(1/a_ig[i],1,1);
for (i in 1:D) sig[i] = sqrt(sig2[i]);
for (i in 1:len_data){
mu[i] = logprem[i]+logelr+alpha[w[i]]+beta[d[i]]*speedup[cal[i]];
}
}
model {
...
while ((max_Rhat > Rhat_target)&(stan_thin<17)){
  fitCSR=stan(model_code=CSRmodel_stan, data = data.CSR,init=init.CSR,
              ...
              refresh=0)
  fitCSR_summary=
    as.matrix(summary(fitCSR)$summary)[1:(W+2*D+2*C),c(1,3,10)]
...
CSR.RetroRes=cumsum(CSR.Estimate)
for (w in 1:W){
  CSR.RetroRes[w]=CSR.RetroRes[w]-sum(adata$ploss[adata$cal==w])
}
# append totals
CSR.SE=round(ss.wdD)
CSR.CV=round(CSR.SE/CSR.Estimate,4)
...
CSR.RetroRes=c(CSR.RetroRes,CSR.RetroRes[W])
Premium=c(Premium,sum(Premium))
AY=c(sort(unique(adata$ay)),"Total")
AY_Exhibit=data.frame(AY,Premium,CSR.Estimate,CSR.SE,CSR.CV,CSR.RetroRes)
print(AY_Exhibit)
Premium=Premium[W+1]
...
abline(h=qnorm(.75))
#
mtext(paste("CSR-vc",run_id, "Standardized Residual Boxplots"),
      side = 3, line = -2.75, outer = TRUE)
#
# plot paid and incurred gamma values
#
gamma_mean=colMeans((gamma[,keep_cal[-length(keep_cal)]]))
rng=range(gamma_mean)
par(mfrow=c(1,1))
plot(keep_cal[-length(keep_cal)],gamma_mean,ylim=rng,type="l",
     lwd=3,col="black",
...
#
Runid=paste("CSR-vc",run_id,"")
Ultimate.Loss=CSR.Estimate
Ultimate.SE=CSR.SE
Reserve.Low=CSR.ResLow
Reserve.Mean=CSR.RetroRes[W]
