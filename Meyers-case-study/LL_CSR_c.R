...
AY.Exhibit=NULL
#AY.Exhibit=paste(run_id,"AY_Exhibit_CSR_c.csv")
parm.summary=NULL
#parm.summary=paste(run_id,"Parm_Summary_CSR_c.csv")
#
# read in data
#
...
init.CSR=function(chain_id){
  ...
  list(r_alpha=rnorm(W-1,0,0.2),r_beta=runif(D-1),a_ig=runif(D),
       logelr=runif(1,-0.75,-0.5),r_gamma=rnorm(1,0,0.1))
}
#
# Stan CSR script
#
...
hist(reserve,
  main=paste("CSR-c",run_id, "Predictive Distribution of the Loss Reserve"),
...
abline(h=qnorm(.75))
#
mtext(paste("CSR-c",run_id, "Standardized Residual Boxplots"),
      side = 3, line = -2.75, outer = TRUE)
#
# write output data files
#
Runid=paste("CSR-c",run_id,"")
